...
const TableOfContent = ({ markdownContent }: TableOfContentProps) => {
  ...
  const tocItems: TOCItem[] = useMemo(() => generateTOC(markdownContent), [markdownContent]);

  // Get section IDs for tracking active section
  const sectionIds = useMemo(() => tocItems.map((item: TOCItem) => item.id), [tocItems]);
              ...
              return (
                <li key={item.id}>
                  <Link
                    ...
                    <span
                      className={`text-lg leading-[27px] transition-all duration-300 ease-out ${indentClass} ${
                        isActive
